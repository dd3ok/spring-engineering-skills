...
Spring과 Spring Boot 엔지니어링을 위한, 특정 벤더에 종속되지 않는 [Agent Skills](https://agentskills.io/specification) 모음입니다. 하나의 거대한 스킬 대신 애플리케이션 구현, 코드 리뷰, 저장소 근거 수집, 업그레이드, 성능, 보안, 테스트, Modulith 경계를 각각 독립된 스킬로 제공합니다.
...
| 스킬 | 사용 목적 | 주 산출물 |
| --- | --- | --- |
| [`spring-application-developer`](skills/spring-application-developer/) | 신규 Spring Boot 애플리케이션과 기존 프로젝트의 코드, 설정, 테스트, migration, integration, 의존성 변경 | 집중 검증을 포함한 실제 저장소 변경 |
...
"분석하고 고쳐줘"처럼 최종 결과가 저장소 변경인 복합 요청은 `spring-application-developer`가 맡아 수정에 필요한 범위만 분석하고 기존 발견 사항을 재사용합니다. 결과가 발견 사항과 개선 조언에서 끝나는 요청은 `spring-engineering-review`가 맡습니다.

## 산출물 동작 방식

8개 스킬 모두 구조화된 결과를 에이전트의 일반 응답으로 반환합니다. 스킬이 활성화되었다는 이유만으로 작업 공간에 파일을 작성하지는 않습니다.

구현 작업에서 실행 권한은 편집 범위와 별개입니다. Maven, Gradle, 테스트, 컨테이너 또는 다른 도구를 실행하지 말라는 명시적 제한은 절대적으로 지키며, 스킬은 요청된 테스트와 설정을 작성하되 실행 검증을 미실행으로 보고할 수 있습니다. 신규 프로젝트의 버전 근거는 정상 TLS 검증으로만 조회하고, 검증된 조회가 실패하면 버전을 추측하지 않습니다.
...
나머지 6개 스킬은 요청과 근거에 따라 달라지는 저장소 변경, 발견 사항, 가설, 위협 모델, 테스트 matrix, 감사 결과를 만듭니다. 여기에 고정된 파일 schema를 강제하면 불필요하게 경직되고 무거워집니다. 요청하면 Markdown이나 JSON으로 저장할 수 있지만, 기본적으로 공식 artifact를 생성하지는 않습니다.
...
스킬 하나를 설치하려면 `skills/<skill-name>/` 전체를 host가 지원하는 스킬 경로로 복사합니다. `SKILL.md`, `LICENSE`, `references/`, 선택 사항인 `scripts/`를 함께 유지하세요. 전체 모음을 설치하려면 8개 스킬 디렉터리를 모두 복사합니다.

요청 예시:

```text
spring-application-developer로 이 Spring Boot 서비스의 제한된 결함 수정과 회귀 테스트를 구현해줘.
...
일반적인 Spring 질문, CVE 조회, 범위가 정해지지 않은 아키텍처 논의에는 이 스킬 모음을 자동으로 활성화하지 않는 것이 좋습니다. 범위가 정해진 Spring 저장소 변경은 `spring-application-developer`가 담당합니다.
