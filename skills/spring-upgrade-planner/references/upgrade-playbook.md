...
Record `spring_cloud_usage` separately from `spring_cloud` so unknown, used, and confirmed non-use cannot collapse into one null. Resolve current evidence and select the exact target train service release, or mark target non-use only when the gate is evidenced as not applicable. For Boot 4.1 with the 2025.1 train, require service release 2025.1.2 or newer. Bind the compatibility snapshot to both exact Boot and Cloud versions.

Record exact target Java and Maven-or-Gradle versions. Every ready source declares its subject and subject version; JDK and build-tool sources must match those target selections and their versioned locator where the vendor provides one.

A SHA-256 establishes the identity of a local source copy, not that it came from the locator. Ready plans require a controlled-capture manifest with the final allowlisted locator, UTC capture time, capture identity, and response hash. The validator checks manifest consistency and allowlists; trust still depends on the stated controlled capture process.
