...
PUBLISHER_SCOPE_PATH_PATTERNS = {
    ...
    ("oracle", "www.oracle.com", "java-migration"): (
...
def nonempty_items(value: object) -> bool:
    return isinstance(value, list) and bool(value) and all(
        (isinstance(item, str) and bool(item.strip())) or (isinstance(item, dict) and bool(item)) for item in value
    )
...
def locator_matches_checked_boot_line(
    path: str, scope: object, checked_version: object, *, allow_mutable: bool
) -> bool:
    ...
    if match is None:
        return allow_mutable and path.startswith("/spring-boot/")
...
def locator_matches_subject_version(path: str, subject: object, subject_version: object) -> bool:
    ...
    if subject == "java":
        ...
        return match is None or int(match.group(1)) == int(subject_version.split(".", 1)[0])
    if subject == "gradle":
...
def valid_capture(
    ...
    *,
    ...
    if not isinstance(capture, dict) or CAPTURE_REQUIRED_FIELDS - set(capture):
        return False
...
def version_key(value: str) -> tuple[int, int, int, int, int]:
    ...
    base = tuple(int(item) for item in base_text.split("."))
    suffix = suffix.upper()
    if not suffix:
        return (*base, 4, 0)
...
def validate(plan: object, base_dir: Path | None = None, *, now: datetime | None = None) -> list[str]:
    ...
    input_source_ids_valid = isinstance(input_data, dict) and isinstance(input_data.get("source_snapshot_ids"), list) and all(isinstance(value, str) for value in input_data.get("source_snapshot_ids", []))
    if not input_source_ids_valid or not SHA256.fullmatch(str(input_data.get("evidence_sha256", ""))):
        errors.append("input evidence hash or source_snapshot_ids are invalid")
    current = plan.get("current")
    target = plan.get("target")
    if (
        not isinstance(current, dict)
        or "spring_boot" not in current
        or not isinstance(current.get("evidence_ids"), list)
    ):
    ...
    if isinstance(current, dict) and (
        ...
        or not isinstance(current.get("spring_cloud_evidence_ids"), list)
        or (current_cloud is not None and (not isinstance(current_cloud, str) or not VERSION.fullmatch(current_cloud)))
    ...
    if not isinstance(target, dict) or not isinstance(target.get("prerelease_allowed"), bool) or not isinstance(target_version, str) or not VERSION.fullmatch(target_version):
        errors.append("target must contain an exact Spring Boot version")
    elif isinstance(input_data, dict) and input_data.get("target") != target_version:
        errors.append("input.target does not match target.spring_boot")
    elif status != "blocked" and re.search(r"-(?:SNAPSHOT|M\d+|RC\d+)$", target_version, re.IGNORECASE) and not target["prerelease_allowed"]:
    ...
    if isinstance(target, dict) and (
        ...
        or (target_cloud is not None and (not isinstance(target_cloud, str) or not VERSION.fullmatch(target_cloud)))
    ...
    if not isinstance(policy, dict) or not isinstance(policy.get("allow_downgrade"), bool):
    ...
    for index, source in enumerate(ledger):
        if not isinstance(source, dict) or SOURCE_REQUIRED_FIELDS - set(source):
            ...
            continue
        if not isinstance(source, dict) or not isinstance(source.get("id"), str) or not source["id"] or source["id"] in source_ids:
        ...
        if status == "ready":
            ...
            if isinstance(scope, str) and scope in {"system-requirements", "managed-dependencies", "spring-upgrade-guide", "build-system-guide"}:
                ...
                if (
                    ...
                    or not locator_matches_checked_boot_line(
                        requested_parsed.path, scope, source.get("checked_version"), allow_mutable=False
                    )
                    or not locator_matches_checked_boot_line(
                        final_parsed.path, scope, source.get("checked_version"), allow_mutable=True
                    )
                ):
            ...
            if (
                requested_parsed is None
                or final_parsed is None
                or not locator_matches_subject_version(requested_parsed.path, subject, subject_version)
                or not locator_matches_subject_version(final_parsed.path, subject, subject_version)
            ):
            ...
            if not valid_snapshot_path(source.get("snapshot_path")) or not SHA256.fullmatch(str(source.get("sha256", ""))):
                ...
                try:
                    ...
                    elif hashlib.sha256(snapshot.read_bytes()).hexdigest() != source["sha256"]:
                        errors.append(f"ready source snapshot hash mismatch: {source['id']}")
    if input_source_ids_valid and set(input_data.get("source_snapshot_ids", [])) != source_ids:
    ...
    for index, gate in enumerate(gates):
        ...
        if status == "ready":
            ...
            if gate["id"] == "gate:java-build-tools" and gate["status"] == "pass":
                required_subjects = {
                    ("spring-boot", str(target_version)),
                    ("java", str(target_java)),
                    (str(target_build_tool), str(target_build_tool_version)),
                }
                referenced_subjects = {
                    source_subject_versions[source_id]
                    for source_id in referenced_ids
                    if source_id in source_subject_versions
                }
                missing_subjects = required_subjects - referenced_subjects
                if missing_subjects:
                    labels = ", ".join(f"{subject}:{version}" for subject, version in sorted(missing_subjects))
    ...
    for index, hop in enumerate(hops):
        ...
        if not isinstance(referenced, list) or not all(isinstance(item, str) for item in referenced) or set(referenced) - source_ids:
    ...
    if not isinstance(plan.get("automation"), list):
        errors.append("automation must be an array")
    rollout = plan.get("rollout")
    if not isinstance(rollout, dict) or not isinstance(rollout.get("canary_signals"), list):
...
def main() -> int:
    ...
    errors = validate(json.loads(args.plan.read_text(encoding="utf-8")), args.plan.parent)
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
