...
2. Verify the requested version line and system requirements before generating files. When the user requests a current, latest, or supported line without an exact version, resolve it to an exact GA from current Spring Initializr metadata and matching official system requirements, record the evidence, and proceed. Ask or stop only when a pinned target is incompatible or pre-release, or when multiple supported choices would materially change the design and the requirements do not resolve them.
