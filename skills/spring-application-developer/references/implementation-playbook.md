...
2. Verify the requested version line and system requirements before generating files. Apply `references/greenfield-baseline-policy.json` as the sole selection algorithm (`spring-initializr-defaults/1`) for every unpinned generation field, including its hash-before-parsing rule. Do not infer a vendor LTS policy. Collect its full provenance for the current run, but render only its compact user summary by default, retain full details in an evaluation trace when the host provides one, and save a metadata snapshot or provenance artifact only when the user requests it. Ask or stop only when the policy blocks or a non-default choice would materially change the requested design.
